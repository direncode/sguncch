...
export async function autoMigrateCodexTables() {
  ...
  if (!databaseUrl) {
    console.warn('[Codex Migration] No DATABASE_URL found. Set DATABASE_URL env var (from Supabase Dashboard → Settings → Database → Connection String → URI) to enable auto-migration.')
    return false
  }
  ...
  try {
    ...
    console.log('[Codex Migration] Tables created successfully — governance_documents, document_chunks, approval_log, form_submissions, funding_requests')
    _migrationSucceeded = true
    return true
  } catch (err) {
    console.error('[Codex Migration] Auto-migration failed:', err.message)
    return false
  }
}
