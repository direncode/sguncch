...
export async function autoMigrateCodexTables() {
  ...
  if (!databaseUrl) {
    console.warn('[Codex Migration] No DATABASE_URL found. Set DATABASE_URL env var or use the admin setup wizard.')
    return false
  }
...
async function runMigrationSQL(connectionString) {
  try {
    ...
    console.error('[Codex Migration] Migration failed:', err.message)
    throw err
  }
}
