...
// SECURITY: Admin keys must be set via environment variables in production
...
// SECURITY: Only use server-side env vars. NEVER use NEXT_PUBLIC_ prefix for secrets.
export const ADMIN_KEYS = {
  [TEAM_ROLES.LEADS]: process.env.ADMIN_KEY_LEADS || process.env.ADMIN_KEY || DEFAULT_DEV_KEY,
  [TEAM_ROLES.OUTREACH]: process.env.ADMIN_KEY_OUTREACH || null,
  [TEAM_ROLES.COMMS]: process.env.ADMIN_KEY_COMMS || null,
  [TEAM_ROLES.STRATEGY]: process.env.ADMIN_KEY_STRATEGY || null,
}

// Backward compatibility
export const ADMIN_KEY = ADMIN_KEYS[TEAM_ROLES.LEADS]
...
if (typeof window === 'undefined' && process.env.NODE_ENV === 'production' && ADMIN_KEY === DEFAULT_DEV_KEY) {
  console.error('SECURITY WARNING: Using default admin key in production! Set ADMIN_KEY_LEADS environment variable.')
}

// Team definitions with metadata
export const adminTeams = [
  { id: TEAM_ROLES.LEADS, name: 'Project Bold Leads', color: '#4B9CD3', description: 'Executive leadership with full platform access', isSuper: true },
  { id: TEAM_ROLES.OUTREACH, name: 'Outreach Team', color: '#F59E0B', description: 'Community engagement, basic needs, wellness, and environmental outreach', isSuper: false },
  { id: TEAM_ROLES.COMMS, name: 'Communications Team', color: '#8B5CF6', description: 'Messaging, content, storytelling, and student voice', isSuper: false },
  { id: TEAM_ROLES.STRATEGY, name: 'Strategy Team', color: '#10B981', description: 'Academic strategy, research, and institutional engagement', isSuper: false },
]

// Default suggested policy mappings for onboarding (teams can customize)
export const defaultTeamPolicySuggestions = {
  [TEAM_ROLES.OUTREACH]: ['wellness', 'basic-needs', 'environmental'], // department IDs
  [TEAM_ROLES.COMMS]: ['communications'],
  [TEAM_ROLES.STRATEGY]: ['academic'],
}

// Helper to get team info by role
export function getTeamByRole(role) {
  return adminTeams.find(t => t.id === role) || null
}

// Helper to check if a role is superadmin
export function isLeadsRole(role) {
  return role === TEAM_ROLES.LEADS
}
...
// Initial budget data - zeroed, aligned with budgetEngine BUDGET_CATEGORIES
export const initialBudgetData = {
  total: 0,
  allocated: 0,
  spent: 0,
  categories: [
    { id: 'events', name: 'Events & Programming', allocated: 0, spent: 0 },
    { id: 'travel', name: 'Travel & Conferences', allocated: 0, spent: 0 },
    { id: 'merch', name: 'Merchandise & Apparel', allocated: 0, spent: 0 },
    { id: 'supplies', name: 'Supplies & Materials', allocated: 0, spent: 0 },
    { id: 'wellness', name: 'Wellness Initiatives', allocated: 0, spent: 0 },
    { id: 'food', name: 'Food & Catering', allocated: 0, spent: 0 },
    { id: 'marketing', name: 'Marketing & Outreach', allocated: 0, spent: 0 },
    { id: 'technology', name: 'Technology & Equipment', allocated: 0, spent: 0 },
    { id: 'emergency', name: 'Emergency & Crisis', allocated: 0, spent: 0 },
