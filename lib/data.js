...
// All progress starts at Phase 1 - Admin advances through phases with narrative updates
...
if (typeof window === 'undefined' && process.env.NODE_ENV === 'production' && ADMIN_KEY === DEFAULT_DEV_KEY) {
  log.error('SECURITY WARNING: Using default admin key in production! Set ADMIN_KEY environment variable.')
}

// ============================================
// POLICY PHASE LIFECYCLE
// ============================================
export const POLICY_PHASES = [
  { number: 1, label: 'Research & Discovery', shortLabel: 'Research', description: 'Gathering data, understanding the problem, surveying students', icon: '1' },
  { number: 2, label: 'Planning & Design', shortLabel: 'Planning', description: 'Drafting proposals, identifying partners, setting goals', icon: '2' },
  { number: 3, label: 'Building Partnerships', shortLabel: 'Partnerships', description: 'Securing commitments from administration, orgs, community', icon: '3' },
  { number: 4, label: 'Implementation', shortLabel: 'Implementing', description: 'Active execution — launching programs, allocating funds', icon: '4' },
  { number: 5, label: 'Monitoring & Iteration', shortLabel: 'Monitoring', description: 'Tracking outcomes, adjusting based on feedback', icon: '5' },
  { number: 6, label: 'Complete & Sustained', shortLabel: 'Complete', description: 'Goal achieved and systems in place for continuity', icon: '6' },
]

export function getPhaseLabel(phaseNumber) {
  const phase = POLICY_PHASES.find(p => p.number === phaseNumber)
  return phase?.label || 'Unknown'
}

export function getPhaseShortLabel(phaseNumber) {
  const phase = POLICY_PHASES.find(p => p.number === phaseNumber)
  return phase?.shortLabel || 'Unknown'
}
...
// Legacy: percentage-based progress (kept for backward compat)
...
// Phase-based progress: average phase (1-6) as percentage of completion
export function getOverallPhaseProgress(policies) {
  if (!policies || policies.length === 0) return 0
  const total = policies.reduce((sum, p) => sum + (p.phase || 1), 0)
  return Math.round((total / policies.length / 6) * 100)
}

// Get phase distribution summary
export function getPhaseSummary(policies) {
  if (!policies) return {}
  const summary = {}
  POLICY_PHASES.forEach(p => {
    summary[p.number] = {
      label: p.label,
      shortLabel: p.shortLabel,
      count: policies.filter(pol => (pol.phase || 1) === p.number).length,
    }
  })
  return summary
}

// Get latest narrative update for a policy
export function getLatestUpdate(policy) {
  if (!policy?.narrativeUpdates?.length) return null
  return policy.narrativeUpdates[0] // Sorted newest first
}
