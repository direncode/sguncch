...
async function useSupabaseCodex() {
  ...
  if (migrated) {
    ...
    try {
      ...
      if (!error) {
        _codexTablesAvailable = true
        console.log('[Codex] Auto-migration succeeded — Supabase storage active')
        return true
      }
    } catch {
      // Fall through
...
// ============================================
// UPDATE DOCUMENT METADATA (query_count, summary, etc.)
// ============================================
export async function updateDocumentMetadata(id, updates) {
  if (await useSupabaseCodex()) {
    const { error } = await supabase
      .from('governance_documents')
      .update(updates)
