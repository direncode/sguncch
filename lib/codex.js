...
import { autoMigrateCodexTables } from './migrations'
...
// Read and parse the committed seed file
function readSeedFile() {
  try {
    if (!fs.existsSync(SEED_FILE)) return null
    const seed = JSON.parse(fs.readFileSync(SEED_FILE, 'utf8'))
    if (!seed) return null
    if (!seed.documents?.length && !seed.chunks?.length && !seed.approval_log?.length) return null
    return seed
  } catch {
    return null
  }
...
// Auto-seed Supabase tables from committed seed file when tables are empty
async function ensureSupabaseSeeded() {
  if (_supabaseSeeded || !supabase) return
  _supabaseSeeded = true

  const seed = readSeedFile()
  if (!seed) return

  try {
    // Check if Supabase tables already have data
    const { data: existing } = await supabase
      .from('governance_documents')
      .select('id')
      .limit(1)

    if (existing && existing.length > 0) return // Already has data, skip seeding

    log.info(`Auto-seeding Supabase from seed file (${seed.documents?.length || 0} documents, ${seed.chunks?.length || 0} chunks)`)

    // Insert documents in batches
    if (seed.documents?.length > 0) {
      for (let i = 0; i < seed.documents.length; i += 50) {
        const batch = seed.documents.slice(i, i + 50)
        const { error } = await supabase
          .from('governance_documents')
          .upsert(batch, { onConflict: 'id' })
        if (error) {
          log.error('Seed documents insert error:', normalizeError(error))
          break
        }
      }
    }
    ...
    if (seed.chunks?.length > 0) {
      for (let i = 0; i < seed.chunks.length; i += 100) {
        const batch = seed.chunks.slice(i, i + 100)
        const { error } = await supabase
          .from('document_chunks')
          .upsert(batch, { onConflict: 'id' })
        if (error) {
          log.error('Seed chunks insert error:', normalizeError(error))
          break
        }
      }
    }

    // Insert approval log entries
    if (seed.approval_log?.length > 0) {
      const { error } = await supabase
        .from('approval_log')
        .upsert(seed.approval_log, { onConflict: 'id' })
      if (error) {
        log.error('Seed approval_log insert error:', normalizeError(error))
      }
    }

    log.info('Supabase auto-seed complete')
  } catch (err) {
    log.error('Supabase auto-seed failed:', err?.message || err)
  }
}
...
// ============================================
// Cache true permanently, but re-check false every 30s so that
// once a user creates tables they start working without a redeploy.
let _codexTablesAvailable = null
let _lastCheckTime = 0
const RECHECK_INTERVAL_MS = 30_000 // re-check every 30 seconds when tables missing

async function useSupabaseCodex() {
  if (!supabase) return false

  // If we know tables exist, keep using them
  if (_codexTablesAvailable === true) return true

  // If we checked recently and tables were missing, don't spam checks
  if (_codexTablesAvailable === false && (Date.now() - _lastCheckTime) < RECHECK_INTERVAL_MS) {
    return false
  }
  ...
  // Table missing — attempt auto-migration via direct Postgres connection
  const migrated = await autoMigrateCodexTables()

  if (migrated) {
    // Verify the table is now accessible through PostgREST
    try {
      const { error } = await supabase
        .from('governance_documents')
        .select('id')
      ...
      if (!error) {
        _codexTablesAvailable = true
        log.info('Auto-migration succeeded — Supabase storage active')
        await ensureSupabaseSeeded()
        return true
      }
    } catch {
...
// Reset the table availability cache (call after successful migration)
export function resetCodexCache() {
  _codexTablesAvailable = null
  _lastCheckTime = 0
  _supabaseSeeded = false
}
...
export async function logApprovalAction(documentId, action, performedBy = 'admin', reason = null) {
  ...
  // File-based fallback
  const entries = readJsonFile(APPROVAL_LOG_FILE)
  entries.push(entry)
  writeJsonFile(APPROVAL_LOG_FILE, entries)
...
export async function getApprovalLog(documentId = null) {
  ...
  // File-based fallback
  let entries = readJsonFile(APPROVAL_LOG_FILE)
  if (documentId) entries = entries.filter(e => e.document_id === documentId)
  entries.sort((a, b) => new Date(b.performed_at) - new Date(a.performed_at))
