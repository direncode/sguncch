...
export async function createDocument({ title, version, source_url, text_full, file_name, file_size }) {
  ...
  if (supabase) {
    try {
      ...
      console.warn('Supabase governance_documents insert failed, using file fallback:', normalizeError(error))
    } catch (e) {
...
export async function getDocuments(status = null, { includeText = false } = {}) {
  if (supabase) {
    try {
      ...
      console.warn('Supabase governance_documents query failed, using file fallback:', normalizeError(error))
    } catch (e) {
      console.warn('Supabase governance_documents unavailable, using file fallback:', e.message)
    }
  }
...
export async function searchDocuments(searchTerm) {
  if (supabase) {
    try {
      ...
      if (!error) return { data: data || [], error: null }
      console.warn('Supabase governance_documents search failed, using file fallback:', normalizeError(error))
    } catch (e) {
      console.warn('Supabase governance_documents unavailable, using file fallback:', e.message)
    }
  }
...
export async function updateDocumentStatus(id, status, approvedBy = 'admin', reason = null) {
  ...
  if (supabase) {
    try {
      ...
      if (!error) return { data: data?.[0], error: null }
      console.warn('Supabase governance_documents update failed, using file fallback:', normalizeError(error))
    } catch (e) {
      console.warn('Supabase governance_documents unavailable, using file fallback:', e.message)
    }
  }
...
export async function insertChunks(documentId, chunks) {
  ...
  if (supabase) {
    try {
      ...
      if (!error) return { data, error: null }
      console.warn('Supabase document_chunks insert failed, using file fallback:', normalizeError(error))
    } catch (e) {
      console.warn('Supabase document_chunks unavailable, using file fallback:', e.message)
    }
  }
...
export async function searchChunksByVector(embedding, limit = 5) {
  ...
  try {
    ...
    if (!error) return { data: data || [], error: null }
    console.warn('Supabase vector search failed:', normalizeError(error))
  } catch (e) {
    console.warn('Supabase vector search unavailable:', e.message)
  }
...
export async function searchChunksByText(query, limit = 5) {
  if (supabase) {
    try {
      ...
      if (!error) return { data: data || [], error: null }
      console.warn('Supabase FTS search failed, using file fallback:', normalizeError(error))
    } catch (e) {
      console.warn('Supabase FTS unavailable, using file fallback:', e.message)
    }
  }
...
export async function deleteDocument(id) {
  if (supabase) {
    try {
      ...
      console.warn('Supabase delete failed, using file fallback:', normalizeError(error))
    } catch (e) {
      console.warn('Supabase delete unavailable, using file fallback:', e.message)
    }
  }
...
export async function logApprovalAction(documentId, action, performedBy = 'admin', reason = null) {
  ...
  if (supabase) {
    try {
      ...
      if (!error) return { data: data?.[0], error: null }
      console.warn('Supabase approval_log insert failed, using file fallback:', normalizeError(error))
    } catch (e) {
      console.warn('Supabase approval_log unavailable, using file fallback:', e.message)
    }
  }
...
export async function getApprovalLog(documentId = null) {
  if (supabase) {
    try {
      ...
      if (!error) return { data: data || [], error: null }
      console.warn('Supabase approval_log query failed, using file fallback:', normalizeError(error))
    } catch (e) {
      console.warn('Supabase approval_log unavailable, using file fallback:', e.message)
    }
  }
