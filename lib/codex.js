...
async function useSupabaseCodex() {
  ...
  console.warn('[Codex] governance_documents table not found — attempting auto-migration...')
  ...
  if (migrated) {
    ...
    try {
      ...
      // PostgREST schema cache may need refresh — table exists but cache is stale
      console.warn('[Codex] Tables created but PostgREST cache stale. Reload the Supabase API schema from the Dashboard (Settings → API → Reload schema).')
    } catch {
      // Fall through
    }
  }
  ...
  console.warn('[Codex] Using file-based storage. To enable persistent Supabase storage, set DATABASE_URL env var (Supabase Dashboard → Settings → Database → Connection String → URI)')
  return false
}
