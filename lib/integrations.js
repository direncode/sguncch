...
export async function submitForm(formType, data) {
  // Rate limit form submissions (10 per minute)
  const rateLimitResult = checkRateLimit(`form_${formType}`, { maxAttempts: 10, windowMs: 60000 })
  if (!rateLimitResult.allowed) {
    return {
      success: false,
      message: `Please wait ${rateLimitResult.resetIn} seconds before submitting again.`,
    }
  }
  ...
  // Check for XSS attempts in any field
  for (const [key, value] of Object.entries(data)) {
    if (typeof value === 'string' && containsXSS(value)) {
      log.warn('Potential XSS detected', { field: key })
      return { success: false, message: 'Invalid input detected. Please check your entries.' }
    }
  }
  ...
  const payload = {
    ...
    subject: `Project Bold - ${sanitizeText(formType)}`,
    from_name: sanitizedData.name || 'Project Bold Form',
    ...sanitizedData,
    form_type: sanitizeText(formType),
    submitted_at: new Date().toISOString(),
  }

  try {
    // Save to Supabase via API (primary persistent storage)
    try {
      await fetch('/api/forms/submit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ formType, data: sanitizedData }),
      })
    } catch (e) {
      log.warn('Supabase form save failed, continuing with Web3Forms', { error: e.message })
    }
    ...
    saveToLocalStorage(formType, sanitizedData)
...
export async function handleFormSubmission(formType, formData, options = {}) {
  ...
  try {
    ...
    if (sendEmail) {
      const emailResult = await submitForm(formType, sanitizedData)
      if (!emailResult.success) {
        results.errors.push(emailResult.message || 'Email submission failed')
      }
    }
    ...
    if (webhookUrl) {
      await sendWebhookNotification(webhookUrl,
        `New ${sanitizeText(formType)} submission from ${sanitizedData.name || 'Anonymous'}`
      )
    }
    ...
    log.error('Form handler error', { error: error.message })
