...
export function AppProvider({ children }) {
  ...
  const loginAdmin = (key) => {
    ...
    if (secureCompare(key, ADMIN_KEY)) {
      ...
      // Create secure session instead of just storing 'true'
  ...
  const submitFundingRequest = (request) => {
    ...
    const { calculateFundingScore } = require('./budgetEngine')
    const aiScore = calculateFundingScore(request, fundingRequests, budgetData, budgetLineItems)
  ...
  const approveFundingRequest = (requestId, approvedAmount, reviewerNotes = '') => {
  ...
  const denyFundingRequest = (requestId, reviewerNotes = '') => {
    setFundingRequests(prev => prev.map(req => {
      if (req.id === requestId) {
        return {
          ...req,
  ...
  const resubmitAmendedRequest = (requestId, acceptAmendment, newAmount, response) => {
    ...
    setFundingRequests(prev => prev.map(req => {
      if (req.id === requestId) {
        ...
        // Recalculate AI score with new amount
        updatedRequest.aiScore = calculateFundingScore(updatedRequest, fundingRequests, budgetData, budgetLineItems)
