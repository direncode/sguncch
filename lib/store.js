...
export function AppProvider({ children }) {
  ...
  const logActivity = useCallback((action, category, details, metadata = {}) => {
    ...
    if (useSupabase) {
      logActivityToSupabase(action, category, details, metadata).catch(err => log.error('Failed to log activity to Supabase', { error: err.message || err }))
    }
  }, [])
    ...
    async function loadData() {
      ...
      if (useSupabase) {
        try {
          ...
          } catch (e) { log.warn('Supabase feedback load failed', { error: e.message || e }) }
          ...
          } catch (e) { log.warn('Supabase activity load failed', { error: e.message || e }) }
          ...
          } catch (e) { log.warn('Supabase announcements load failed', { error: e.message || e }) }
          ...
          } catch (e) { log.warn('Supabase policies load failed', { error: e.message || e }) }
          ...
          } catch (e) { log.warn('Supabase budget load failed', { error: e.message || e }) }
          ...
          } catch (e) { log.warn('Supabase quick stats load failed', { error: e.message || e }) }
          ...
          } catch (e) { log.warn('Supabase funding requests load failed', { error: e.message || e }) }
          ...
          } catch (e) { log.warn('Supabase tech devices load failed', { error: e.message || e }) }
          ...
          } catch (e) { log.warn('Supabase pantry load failed', { error: e.message || e }) }
          ...
          log.error('Error loading from Supabase, falling back to localStorage', { error: e.message || e })
      ...
      try {
        ...
        log.error('Error loading from localStorage', { error: e.message || e })
      }
  ...
  const advancePhase = (policyId, narrativeUpdate) => {
    const policy = policies.find(p => p.id === policyId)
    if (!policy) return
    if (!narrativeUpdate?.title || !narrativeUpdate?.narrative) return
    ...
    const update = {
      ...
      phase: nextPhase,
      title: narrativeUpdate.title,
      narrative: narrativeUpdate.narrative,
      evidence: narrativeUpdate.evidence || [],
      impact: narrativeUpdate.impact || '',
      author: narrativeUpdate.author || 'Admin',
    }
    ...
    setPolicies(prev => prev.map(p =>
      p.id === policyId ? {
        ...p,
        phase: nextPhase,
        progress: Math.round((nextPhase / 6) * 100),
        narrativeUpdates: [update, ...(p.narrativeUpdates || [])],
        status: nextPhase >= 6 ? 'completed' : nextPhase >= 2 ? 'in_progress' : 'planned',
        lastUpdated: new Date().toISOString(),
      } : p
    ))

    logActivity('ADVANCE_PHASE', policy.department, `Advanced "${policy.title}" to Phase ${nextPhase}`, {
      policyId, phase: nextPhase, updateTitle: update.title,
    })
  }

  const addNarrativeUpdate = (policyId, update) => {
    const policy = policies.find(p => p.id === policyId)
    if (!policy) return
    ...
    setPolicies(prev => prev.map(p =>
      p.id === policyId ? {
        ...p,
        narrativeUpdates: [entry, ...(p.narrativeUpdates || [])],
        lastUpdated: new Date().toISOString(),
      } : p
    ))

    logActivity('NARRATIVE_UPDATE', policy.department, `Added update to "${policy.title}": ${entry.title}`, {
      policyId, updateTitle: entry.title,
    })
  }

  const updateImpactSummary = (policyId, summary) => {
    setPolicies(prev => prev.map(p =>
      p.id === policyId ? { ...p, impactSummary: summary, lastUpdated: new Date().toISOString() } : p
    ))
  }

  const updateOutcomes = (policyId, outcomes) => {
    setPolicies(prev => prev.map(p =>
      p.id === policyId ? { ...p, outcomes: { ...(p.outcomes || {}), ...outcomes }, lastUpdated: new Date().toISOString() } : p
    ))
  }
  ...
  const addAnnouncement = async (announcement) => {
    ...
    if (useSupabase) {
      try {
        ...
        log.error('Supabase announcement creation failed', { error: e.message || e })
      }
    }
  ...
  const updateAnnouncement = async (id, updates) => {
    ...
    if (useSupabase) {
      try {
        ...
        log.error('Supabase announcement update failed', { error: e.message || e })
      }
    }
  ...
  const deleteAnnouncement = async (id) => {
    ...
    if (useSupabase) {
      try {
        ...
        log.error('Supabase announcement deletion failed', { error: e.message || e })
      }
    }
  ...
  const submitFeedback = async (feedbackData) => {
    ...
    if (useSupabase) {
      try {
        ...
        log.error('Supabase feedback submission failed, using localStorage', { error: e.message || e })
      }
    }
  ...
  const updateFeedbackStatus = async (id, status, adminNote = '') => {
    ...
    if (useSupabase) {
      try {
        ...
        log.error('Supabase feedback update failed', { error: e.message || e })
      }
    }
  ...
  const submitFundingRequest = async (request) => {
    ...
    try {
      ...
      log.info('GROQ validation skipped', { error: e.message })
    }
    ...
    if (useSupabase) {
      try {
        ...
        log.warn('Supabase funding request save failed, using localStorage', { error: e.message || e })
      }
    }
  ...
  const approveFundingRequest = async (requestId, approvedAmount, reviewerNotes = '') => {
    ...
    if (useSupabase) {
      ...
      } catch (e) { log.warn('Supabase funding request update failed', { error: e.message || e }) }
    }
