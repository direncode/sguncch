...
import { initialPolicies, initialOperationalData, initialBudgetData, TEAM_ROLES, defaultTeamPolicySuggestions, getTeamByRole, isLeadsRole } from './data'
import {
  supabase,
...
import { setAdminToken, clearAdminToken, setAdminRole as setSessionRole, clearAdminRole as clearSessionRole, getAdminRole as getSessionRole, setAccountInfo as setSessionAccountInfo, getAccountInfo as getSessionAccountInfo, clearAccountInfo as clearSessionAccountInfo } from './adminSession'
...
export function AppProvider({ children }) {
  ...
  const [adminRole, setAdminRole] = useState(null) // 'leads', 'outreach', 'comms', 'strategy'
  ...
  // Team policy mapping: { teamRole: [policyId, ...] }
  const [teamPolicyMap, setTeamPolicyMap] = useState({})
  // Track which teams have completed onboarding
  const [teamOnboardingComplete, setTeamOnboardingComplete] = useState({})

  // Admin account info (persistent user account)
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        // Load team data (always from localStorage regardless of Supabase)
        const savedTeamPolicyMap = localStorage.getItem('projectbold_team_policy_map')
        const savedTeamOnboarding = localStorage.getItem('projectbold_team_onboarding')
        if (savedTeamPolicyMap) setTeamPolicyMap(JSON.parse(savedTeamPolicyMap))
        if (savedTeamOnboarding) setTeamOnboardingComplete(JSON.parse(savedTeamOnboarding))

        // Restore admin role from session if still logged in
        const savedRole = getSessionRole()
        if (savedRole) setAdminRole(savedRole)

        // Restore account info from session
        const savedAccount = getSessionAccountInfo()
        if (savedAccount) setAccountInfoState(savedAccount)
      } catch (e) {
  ...
  const loginAdmin = async (key) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const role = data.role || TEAM_ROLES.LEADS
        setIsAdmin(true)
        setAdminRole(role)

        // Store the entered key and role for use in admin API calls
        setAdminToken(key)
        setSessionRole(role)
        ...
        // Check if account exists and store it
        if (data.account) {
          const acct = {
            id: data.account.id,
          ...
          setAccountInfoState(acct)
          setSessionAccountInfo(acct)
        }

        const teamInfo = getTeamByRole(role)
        logActivity('LOGIN', 'auth', `${teamInfo?.name || 'Admin'} logged in`, { role })
        return { success: true, role, hasAccount: !!data.account }
      }
  ...
  const tempLoginAdmin = () => {
    ...
    logActivity('TEMP_LOGIN', 'auth', 'Admin logged in via temp access (Leads)', { role })
    return { success: true, role }
  }

  const logoutAdmin = () => {
    logActivity('LOGOUT', 'auth', 'Admin logged out', { role: adminRole })
    setIsAdmin(false)
    setAdminRole(null)
    setEditMode(false)
    setAccountInfoState(null)

    // Clear secure session, admin token, role, and account info
    if (typeof window !== 'undefined') {
      clearSession()
      clearAdminToken()
      clearSessionRole()
      clearSessionAccountInfo()
      localStorage.removeItem('projectbold_admin')
    }
  }

  // Set account info after registration
  const setAccountInfo = (account) => {
    setAccountInfoState(account)
    setSessionAccountInfo(account)
  }
  ...
  // Save team's policy selections (from onboarding or reassignment)
  const saveTeamPolicyMap = useCallback((role, policyIds) => {
    setTeamPolicyMap(prev => {
      const updated = { ...prev, [role]: policyIds }
      if (typeof window !== 'undefined') {
        localStorage.setItem('projectbold_team_policy_map', JSON.stringify(updated))
      }
      return updated
    })
    // Mark onboarding as complete for this team
    setTeamOnboardingComplete(prev => {
      const updated = { ...prev, [role]: true }
      if (typeof window !== 'undefined') {
        localStorage.setItem('projectbold_team_onboarding', JSON.stringify(updated))
      }
      return updated
    })
    logActivity('TEAM_ONBOARDING', 'system', `${getTeamByRole(role)?.name || role} completed policy selection`, { role, policyCount: policyIds.length })
  }, [logActivity])

  // Get policies for the current team (Leads get all)
  const getTeamPolicies = useCallback((role) => {
    if (!role || isLeadsRole(role)) return policies
    const policyIds = teamPolicyMap[role]
    if (!policyIds || policyIds.length === 0) return []
    return policies.filter(p => policyIds.includes(p.id))
  }, [policies, teamPolicyMap])

  // Check if current team needs onboarding
  const needsOnboarding = useMemo(() => {
    if (!adminRole || isLeadsRole(adminRole)) return false
    return !teamOnboardingComplete[adminRole]
  }, [adminRole, teamOnboardingComplete])

  // Computed: is current user a Leads superadmin?
  const isLeads = useMemo(() => isLeadsRole(adminRole), [adminRole])
