...
export function quickPriceCheck(category, amount, description, studentsImpacted = null) {
  ...
  // Check per-student cost if students impacted is provided
  if (studentsImpacted && studentsImpacted > 0) {
    const perStudentCost = amount / studentsImpacted
    if (perStudentCost > benchmark.perStudent * 2) {
      flags.push(`Per-student cost ($${perStudentCost.toFixed(2)}) is more than 2x the typical $${benchmark.perStudent}/student for ${category}`)
      severity = 'high'
    } else if (perStudentCost > benchmark.perStudent) {
      flags.push(`Per-student cost ($${perStudentCost.toFixed(2)}) exceeds typical $${benchmark.perStudent}/student for ${category}`)
  ...
  return {
    ...
    flags,
    severity,
    perStudentCost: studentsImpacted ? (amount / studentsImpacted).toFixed(2) : null,
