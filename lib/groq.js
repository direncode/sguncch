...
export async function validateFundingRequest(request) {
  ...
  console.log('[Grok-Budget] validateFundingRequest called for:', request.orgName, '$' + request.amount)
  console.log('[Grok-Budget] XAI_API_KEY configured:', apiKey ? 'Yes' : 'No')

  if (!apiKey) {
    console.warn('[Grok-Budget] XAI_API_KEY not set — skipping context validation')
    return {
      validated: true,
      skipped: true,
      isRealistic: true,
  ...
  console.log('[Grok-Budget] Sending request to xAI Grok...')
...
You must respond with valid JSON only, no markdown or explanation outside the JSON. Use this exact format:
{
  ...
  "note": "One sentence summary"
    ...
    console.log('[Grok-Budget] Response status:', response.status)

    if (!response.ok) {
      const errorText = await response.text()
      console.error('[Grok-Budget] API error:', response.status, errorText)
      return {
        validated: true,
        skipped: true,
        isRealistic: true,
    ...
    console.log('[Grok-Budget] Response received, content length:', content?.length || 0)

    if (!content) {
      console.warn('[Grok-Budget] Empty content in response')
      return {
        validated: true,
        skipped: true,
        isRealistic: true,
    ...
    try {
      ...
      console.log('[Grok-Budget] Successfully parsed response:', analysis.flag)
      return {
        validated: true,
        skipped: false,
      ...
      console.error('[Grok-Budget] Failed to parse response as JSON:', parseError.message)
      console.error('[Grok-Budget] Raw content:', content.substring(0, 500))
      return {
        validated: true,
        skipped: true,
        isRealistic: true,
    ...
    console.error('[Grok-Budget] Request failed:', error.name, error.message)
    return {
      validated: true,
      skipped: true,
      isRealistic: true,
