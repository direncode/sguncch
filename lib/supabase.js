...
export async function createAdminAccount(accountData) {
  ...
  const { data, error } = await supabase
    ...
    .insert([{
      ...
      team_role: accountData.teamRole,
      password_hash: accountData.passwordHash,
      avatar_color: accountData.avatarColor || '#4B9CD3',
      bio: '',
      created_at: new Date().toISOString(),
...
export async function getAdminAccountByClerkId(clerkId) {
  if (!supabase) return { data: null, error: 'Supabase not configured' }
...
export async function linkClerkIdToAccount(accountId, clerkId, email) {
  if (!supabase) return { error: 'Supabase not configured' }

  const updates = { clerk_id: clerkId }
...
export async function getAdminAccountByRole(role) {
  if (!supabase) return { data: null, error: 'Supabase not configured' }
...
export async function getAllAdminAccounts() {
  ...
  const { data, error } = await supabase
    .from('admin_accounts')
    .select('id, username, display_name, team_role, avatar_color, bio, created_at, last_login')
    .order('created_at', { ascending: true })
...
// POLICY DISCUSSIONS
// ============================================
export async function getDiscussions(policyId) {
  if (!supabase) return { data: [], error: 'Supabase not configured' }
...
export async function postDiscussion(discussionData) {
  if (!supabase) return { data: null, error: 'Supabase not configured' }

  const { data, error } = await supabase
    .from('policy_discussions')
    .insert([{
      policy_id: discussionData.policyId,
      user_id: discussionData.userId,
      username: discussionData.username,
      display_name: discussionData.displayName,
      team_role: discussionData.teamRole,
...
// ============================================
// CHAT HISTORY (Grok conversation persistence)
// ============================================
...
export async function saveChatConversation(userId, conversationId, title, messages) {
  if (!supabase) return { data: null, error: 'Supabase not configured' }
